...
class Solver:
  def __init__(self):
    ...
    # setup training graphs and summaries，多gpu训练
  ...
  def _tower_loss(self, train_batch):
    ...
    # model and loss function，构建模型和损失函数
  ...
  def train_and_eval(self):
    ...
    # tf.ConfigProto()：tensorflow用于参数的配置
    ...
    with tf.Session(config=sess_config) as sess:
      ...
      # 从预训练的模型中finetune
        ...
        # 从一个列表中取出所有的元素，tf.get_collection().GLOBAL_VARIABLES: 该collection默认加入所有的Variable对象，并且在分布式环境中共享。
